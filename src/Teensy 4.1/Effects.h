...
class Gain : public Effect {
    ...
    public:
        Gain(float gainDB = 0.0f) { setGain(gainDB); }
...
class Scrubby : public Effect {
    ...
    private:
        ...
        const size_t bufSize = (size_t)8 * (size_t)SAMPLE_RATE; // 8s

        float* inBuf = nullptr; 
        size_t writePos = 0; float readPos = 0.0f;

        float mix, seekRateLow, seekRateHigh, seekRange, seekDurLow, seekDurHigh;
        int octavesDown, octavesUp;
        ...
        float constrainSpeed(float speed) { return std::clamp(speed, powf(2.0f, (float)octavesDown), powf(2.0f, (float)octavesUp)); }
    ...
    public:
        ...
        ~Scrubby() { if (inBuf) { extmem_free(inBuf); inBuf = nullptr; } }
        ...
        void setSeekRate(float seekRateLow, float seekRateHigh) { // Hz, [0.3, 810.0] for both
            ...
            seekInterval = 1.0f / ((seekRateHigh + seekRateLow) / 2.0f); // Init interval to average
        }
        void setSeekRange(float seekRange) { this->seekRange = std::clamp(seekRange, 0.3f, 6000.0f); } // ms, [0.3, 6000.0]
    ...
    void process(const float* in, float* out, size_t n) override {
            for (size_t i = 0; i < n; ++i) {
                ...
                out[i] = dryWetMix(in[i], wetSig, mix);
...
class SpectralGate : public Phase_Vocoder {
    ...
    public:
        SpectralGate(float mix = 1.0f, float thresholdDB = -10.0f, float tilt = 0.5f, 
            size_t fftSize = 512, size_t hopFactor = 4) : Phase_Vocoder(mix, fftSize, hopFactor)
