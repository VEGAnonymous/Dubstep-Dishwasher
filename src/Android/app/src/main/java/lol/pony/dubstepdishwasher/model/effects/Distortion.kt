...
class Distortion(id: Int) : Effect() {
    ...
    override val parameters = mutableListOf<EffectParameter<*>>(
        ...
        EffectParameter.Range(id = 2, name = "Drive", unit = ParamUnit.PERCENT, range = 0.0 to 1.0f, exp = 1f, step = 0.01f, initialValue = 0.25f),
        EffectParameter.Toggle(id = 3, name = "AAF", initialValue = false)
    )
}