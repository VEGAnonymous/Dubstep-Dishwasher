...
class BleManager(private val context: Context) {
    ...
    /**
     * Searches for all writable characteristics
     * of the [connectedDevice].
     */
    fun discoverCharacteristics() {
        connection?.discoverServices()
            ?.subscribe({ services ->
                // gets all writable characteristics
                val writableCharacteristics = services.bluetoothGattServices
                    .flatMap { it.characteristics }
                    .filter { (it.properties and (BluetoothGattCharacteristic.PROPERTY_WRITE or BluetoothGattCharacteristic.PROPERTY_WRITE_NO_RESPONSE)) != 0 }
                characteristics.value = writableCharacteristics
                // reads initial value
     ...
     * Updates the [characteristicsData] map with the new value.
     *
     ...
     */
    fun readCharacteristic(characteristicUuid: UUID, expectedValue: ByteArray? = null) {
        connection?.readCharacteristic(characteristicUuid)
            ?.subscribe({ value ->
                val newMap = characteristicsData.value.toMutableMap()
                val hexString = bytesToHexString(value)
                // verifying stored value
                if (expectedValue != null) {
                    if (value.contentEquals(expectedValue)) {
                        newMap[characteristicUuid] = hexString
                    } else {
                        newMap[characteristicUuid] = "$hexString (Invalid)"
                    }
                } else {
                    // stores value in map
                    newMap[characteristicUuid] = hexString
                }
                characteristicsData.value = newMap
            }, { throwable ->
                val newMap = characteristicsData.value.toMutableMap()
                // update map with read error message
                newMap[characteristicUuid] = "Error: ${throwable.message}"
                characteristicsData.value = newMap
            })
            ?.let { compositeDisposable.add(it) }
    }
     ...
     */
    fun writeCharacteristic(value: String, characteristicUuid: UUID) {
        try {
            // converts string to identical byte array
            val bytesToWrite = hexStringToByteArray(value)
            connection?.writeCharacteristic(characteristicUuid, bytesToWrite)
                ?.subscribe({
                    // reads value and verifies it if readable characteristic
                    val characteristic = characteristics.value.find { it.uuid == characteristicUuid }
                    val isReadable = characteristic?.properties?.and(BluetoothGattCharacteristic.PROPERTY_READ) != 0
                    if (isReadable) {
                        readCharacteristic(characteristicUuid, bytesToWrite)
                    } else {
                        // updates map manually if unreadable
                        val newMap = characteristicsData.value.toMutableMap()
                        newMap[characteristicUuid] = bytesToHexString(bytesToWrite)
                        characteristicsData.value = newMap
                    }
                }, { throwable ->
                    val newMap = characteristicsData.value.toMutableMap()
                    // updates map with write error message
                    newMap[characteristicUuid] = "Write Error: ${throwable.message}"
                    characteristicsData.value = newMap
                })
                ?.let { compositeDisposable.add(it) }
            ...
            val newMap = characteristicsData.value.toMutableMap()
            newMap[characteristicUuid] = "Invalid Hex String"
            characteristicsData.value = newMap
        }
    }
